...
def create_boxplot_and_heatplot(model, data, x_axis_label,  savename):
    ...
    legth_values = len(values)
    length_image = len(sens_matrix)
    median = int(np.median(range(0, legth_values)))
    sens_ij_nm = []
    for j in range(0, length_image, legth_values):
        per_protein1 = []
        for i in range(0, length_image, legth_values):
            if i == j:
                continue
            tmp = sens_matrix[j:j + legth_values, i:i + legth_values].copy()
    ...
    im = ax1.imshow(sens_matrix, interpolation='nearest', origin='lower', cmap=plt.get_cmap(colors), vmin=v_min, vmax=v_max,
                    extent=[0, length_image, 0, length_image])
    shape_label = np.arange(legth_values/2, length_image, legth_values)
    ...
    xticks = ([i for i in range(0, length_image, legth_values)])
    ...
    if savename == 'ras_sensitivity_boxplot.eps':
